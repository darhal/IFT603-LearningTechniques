...
class ModelTester:
    ...
    def __init__(self, class_name):
        ...
        self.models = [model_class(stand_trans=config)
    ...
    def train(self, train_set, show_graph=True):
        ...
        if show_plot:
            fig, ax = plt.subplots(
                len(components), len(self.models), figsize=(14, 8), dpi=120
            )
        ...
        for ax_idx, model in enumerate(self.models):
            ...
            perf_matrix.append(
                get_performance_metrics(classes, train_set.labels, probs)
            )
        if show_plot:
            fig.tight_layout()
            plt.show()
    ...
    def predict(self, test_set):
        ...
        for i, m in enumerate(self.models):
            ...
            perf_matrix.append(get_performance_metrics(
                classes, test_set.labels, probs))
    ...
    def build_perf_dataframe(self, perf_matrix, title=""):
        ...
        return pd.DataFrame(
            ...
            columns=[
                f"[{title}] {self.class_name} (Stand={config})"
                for config in self.model_configs
            ],
            index=[
                "Accuracy",
    ...
    def visualise_learning_curve(self, dataset):
        ...
        for ax_idx, estimator in enumerate(self.models):
            LearningCurveDisplay.from_estimator(
                estimator.model, **common_params, ax=ax[ax_idx]
            )
